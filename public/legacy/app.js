(function () {
  ...
  let preserveStoredProgress = requestedView === "store";
  let runtime = getRuntimeForView(loadRuntime());
  ...
  function hasMeaningfulProgressForRuntime(candidate) {
    return (
      candidate.currentEndingId !== null ||
      candidate.currentSceneId !== STORY.startSceneId ||
      Object.keys(candidate.flags).length > 0 ||
      candidate.screen === "intro" ||
      candidate.screen === "chapter" ||
      candidate.screen === "scene" ||
      candidate.screen === "ending"
    );
  }

  function getRuntimeForView(candidate) {
    const safeRuntime = sanitizeRuntime(candidate);
    ...
    if (requestedView === "story") {
      if (hasMeaningfulProgressForRuntime(safeRuntime)) {
        return {
          ...safeRuntime,
          screen: safeRuntime.currentEndingId ? "ending" : "scene"
        };
      }
  ...
  function saveRuntime() {
    if (runtime.screen === "start") {
      if (!preserveStoredProgress) {
        window.localStorage.removeItem(STORAGE_KEY);
      }
      return;
    }

    preserveStoredProgress = false;
  ...
  function hasMeaningfulProgress() {
    return hasMeaningfulProgressForRuntime(runtime);
  }
