...
- Hard custom keys are limited to `object_anchor`, `prop_anchor`, `locked_object`, and sparingly `required_material`. `proof_object` / `beweisobjekt`, `alltagswaffe`, `ersetzungsmoment`, `false_friend_signal`, and `kindmoment` / `mila_kindmoment` are soft material/pressure guidance.
