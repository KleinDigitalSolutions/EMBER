...
- `authorIntent` and `currentFocus` live in the Master Brief JSON and are soft prompt controls: `authorIntent` is the long-term book promise, `currentFocus` is the next 1-3 scene focus. They should guide drafting without overriding Scene Card hard constraints.
